...
class QtSegmentItem(BaseQtPreviewItem):
    ...
    def boundingRect(self):
        ...
        """
        if self.value[0]!=None and self.value[1]!=None :
            x=min(self.value[0].x(), self.value[1].x())-100.0
            y=min(self.value[0].y(), self.value[1].y())-100.0
            d1=abs(self.value[0].x()-self.value[1].x())**2
            d2=abs(self.value[0].y()-self.value[1].y())**2
            d=math.sqrt(d1+d2)**2
